...
##This weird name, artist syntax seems to work. Trying to properly do track:{track} and artist:{artist} is unreliable.
def search_by_name_and_artist(name,artist):
    track_tuples = []
    track_results = sp.search(q=f"name:{name}, {artist}", limit=1,offset=0)
    print(track_results)
